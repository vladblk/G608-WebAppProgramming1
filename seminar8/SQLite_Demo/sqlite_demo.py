...
# conn.commit()
#
# # adding the values into a variable
# user = ('00002', 'Louise', 'Lane', 'Female')
#
...
# # add multiple users
# more_users = [
#     ('00003', 'John', 'Snow', 'Male'),
#     ('00004', 'Marie', 'Curie', 'Female')
# ]
#
...
# DELETING DATA
cur.execute("DELETE FROM users WHERE last_name='Parker';")
conn.commit()

# if every user with last name of Parker was deleted, this should return an empty list []
cur.execute("SELECT * FROM users WHERE last_name='Parker';")
print(cur.fetchall())

# JOINING TABLES
# generate a query that includes the first and last name for each customer on each order.
cur.execute("""
    SELECT *, users.first_name, users.last_name FROM orders
